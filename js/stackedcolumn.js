...
var energy_production_types = ["Stein- und Braunkohle","Rohöl","Naturgas","Kernenergie","Sonnenenergie","Biomasse und Abfälle","Geothermische Energie","Wasserkraft","Windkraft"];
...
if (chart_data_type == 1){
	...
	for (x in energy){
		series[0].data.push(energy[x].production_coal_absolute);
		series[1].data.push(energy[x].production_oil_absolute);
		series[2].data.push(energy[x].production_gas_absolute);
		series[3].data.push(energy[x].production_core_absolute);
		series[4].data.push(energy[x].production_renewable_solar_absolute);
		series[5].data.push(energy[x].production_renewable_waste_absolute);
		series[6].data.push(energy[x].production_renewable_geothermic_absolute);
		series[7].data.push(energy[x].production_renewable_water_absolute);
		series[8].data.push(energy[x].production_renewable_wind_absolute);		
	}
